...
C3 never filters itself by zone_family — the selected zone(s) are
highlighted (full opacity), others dimmed, but all 6 always render. See
PLAN_UI.md "never filter yourself" rule. Multi-select: zero or more zones
can be selected at once (plain click toggles a zone in/out).
"""
...
def build_zone_boxplots(
    ...
    for zone in ZONE_ORDER:
        ...
        active = not selected_zones or zone in selected_zones
        fig.add_trace(
            go.Box(
                x=[zone],
                q1=[s["q1"]],
